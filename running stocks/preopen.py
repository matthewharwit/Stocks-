import sys
sys.stdout = open('output_log_preopen.txt', 'w')
sys.stderr = open('error_log_preopen.txt', 'w')
...
STOP_PERCENT = 0.15 #Seems with most gap %'s, 0.13 gives 10% stop loss - when ticker gaps too much i.e. closer to 10%, stop loss is ~5%. Most tickers don't gap >5% though. So it may actually hurt gains.   
MARGIN_PERCENT = 0.35
order_multiplier = 1.5 #at 3.0 and all else equal, didn't get full fill. 
...
total_big_filtered = len(big_and_filtered)

#added this parameter because it appears that on days with few gaps, a lot of them don't end up making the final list, so I am way under-leveraged.
#Will consider doing this at *4 for <11 tickers. Monitor this first and see how effective it is.

if total_big_filtered < 16:
    order_multiplier = order_multiplier * 2
...
for ticker in filtered_up_list:
      ...
      try:
        ...
        #all of them were 2.5%, it still takes that and divides it by 1.3, making it larger, AND it assumes all tickers get filled.
        ...
        order = Order()
        order.action = 'SELL'
        order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 1.025) )/(0.65) )
        order.orderType = 'LMT'
        order.tif = "OPG"
        ...
        # STOP LOSS
        # Stop loss messed up, should be at 10%, stopping out at ~ 7%. Reason: Assumes again that previous day close of ticker *1.025 is opening price of ticker.
        # That simply is not the case most of the time (as some tickers gap more than 2.5%), so it will get filled earlier than actual stop. 
        stop_order = Order()
        stop_order.action = 'BUY'
        stop_order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 1.025) )/(0.65) )
        ...
        exit_order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 1.025) )/(0.65) )
...
for ticker in down_list:
  if ticker in margin_dict_maint and ticker in margin_dict_init:
    if margin_dict_maint[ticker] < MARGIN_PERCENT and margin_dict_init[ticker] < MARGIN_PERCENT:
      filtered_down_list.append(ticker)
'''
...
total_big_filtered_down = len(big_and_filtered_down)
...
#         order = Order()
#         order.action = 'BUY'
#         order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered_down) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 0.975) )/(0.65 * 2) )
#         order.orderType = 'LMT'
#         order.tif = "OPG"
...
#         stop_order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered_down) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 0.975) )/(0.65 * 2) )
#         stop_order.orderType = 'STP'
...
#         exit_order.totalQuantity = int( ( (order_multiplier * original_balance / total_big_filtered_down) / (previous_day[previous_day['Symbol'] == ticker].close.iloc[0] * 0.975) )/(0.65 * 2) )
#         exit_order.orderType = 'MOC'
#         exit_order.eTradeOnly = False
#         exit_order.firmQuoteOnly = False
#         exit_order.orderId = app.nextorderid
...
pd.Series(filtered_up_list).to_csv('ordered_tickers_up.csv', index=False)
pd.Series(filtered_down_list).to_csv('ordered_tickers_down.csv', index=False)
up_quantity =  order_multiplier * original_balance / total_big_filtered
pd.Series(up_quantity).to_csv('up_quantity.csv', index=False)
down_quantity = order_multiplier * original_balance / total_big_filtered_down
pd.Series(down_quantity).to_csv('down_quantity.csv', index=False)
